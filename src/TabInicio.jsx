...
export default function TabInicio({ usuario, esAdmin, esPublico, t, onLogin, reservas = [] }) {
  ...
  function proximoLibreInfo(consultorio) {
    ...
    if (totalOcupadas === 0) return { texto: "Libre 8:00 – 22:00", color: "#4fc3f7", bg: "rgba(79,195,247,0.12)" };
    const bloques = getBloquesLibres(consultorio);
    const futuros = bloques.filter(b => b.hasta > horaActual);
    if (futuros.length === 0) return { texto: "Sin más turnos hoy", color: "#ef5350", bg: "rgba(239,83,80,0.1)" };
    const b = futuros[0];
    const desde = Math.max(b.desde, horaActual);
    if (desde === horaActual) return { texto: `Libre ahora – ${b.hasta}:00`, color: "#66bb6a", bg: "rgba(102,187,106,0.12)" };
    return { texto: `Libre ${desde}:00 – ${b.hasta}:00`, color: "#4fc3f7", bg: "rgba(79,195,247,0.12)" };
  }
    ...
    <div ref={scrollRef} style={{ height: "100vh", overflowY: "auto", background: "#000000" }} className="tab-content">
      ...
      <div style={{ background: "linear-gradient(180deg,#0a0a14 0%,#000000 100%)", padding: "54px 20px 24px" }}>
        ...
        <div style={{
          ...
          transform: `translateY(${-logoProgress * 12}px)`,
          pointerEvents: logoProgress > 0.8 ? "none" : "auto",
        }}>
          <img src="/logohead.jpeg" alt="GRINS" style={{ height: 48, objectFit: "contain", opacity: 0.95 }} />
        </div>
        ...
        {!esPublico && (
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 4 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: "#7c6aff", fontWeight: 500, marginTop: 3 }}>
      ...
      <div style={{ padding: "16px 14px 100px" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 10 }}>
            {[
              ...
              >
                {tile.icon}
                <span style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.8)", textAlign: "center" }}>{tile.label}</span>
              </button>
            ))}
          </div>
          ...
          {!yaInstalado && (
            <button onClick={handleInstalar} style={{
              ...
              onTouchEnd={e => e.currentTarget.style.opacity = "1"}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7c6aff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 11, color: "#4a5270" }}>{new Date().toLocaleDateString("es-AR", { weekday: "long", day: "numeric", month: "short" })}</span>
          </div>

          <div style={{ background: "rgba(14,12,28,0.8)", borderRadius: 18, overflow: "hidden", border: "1px solid rgba(124,106,255,0.12)", backdropFilter: "blur(12px)" }}>
            {CONSULTORIOS.map((consultorio, ci) => {
              const info = proximoLibreInfo(consultorio);
              return (
                <div key={consultorio} style={{ padding: "14px 16px", borderBottom: ci < 2 ? "1px solid rgba(124,106,255,0.08)" : "none" }}>
                      ...
                      return (
                        <div key={h} style={{ flex: 1, position: "relative" }}>
                          ...
                          <div style={{
                            ...
                            background: ocupado ? color : esPasada ? "rgba(255,255,255,0.05)" : "rgba(124,106,255,0.2)",
                            border: esActual ? "1px solid rgba(255,255,255,0.4)" : ocupado ? "none" : "1px solid rgba(124,106,255,0.15)",
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5 }}>
                    {["8h", "11h", "14h", "17h", "20h"].map(h => (
                      <span key={h} style={{ fontSize: 9, color: "#3a3a5a", fontWeight: 600 }}>{h}</span>
                    ))}
                  </div>
                </div>
              );
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 8, paddingLeft: 4 }}>
            {[["rgba(124,106,255,0.3)", "Libre"], ["#667eea", "Ocupado"], ["rgba(255,255,255,0.06)", "Pasado"]].map(([bg, label]) => (
              <div key={label} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <div style={{ width: 10, height: 10, borderRadius: 3, background: bg }} />
                <span style={{ fontSize: 10, color: "#4a5270", fontWeight: 500 }}>{label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {mensajes.map(m => (
          <div key={m.id} style={{ background: "rgba(14,12,28,0.8)", borderRadius: 14, padding: "12px 14px", marginBottom: 10, border: "1px solid rgba(124,106,255,0.08)", backdropFilter: "blur(12px)" }}>
            ...
            <p style={{ margin: 0, fontSize: 13, color: "#a0a8c0", lineHeight: 1.6 }}>{m.texto}</p>
          </div>
        ))}
      ...
      {mostrarIOSGuide && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 9999, display: "flex", alignItems: "flex-end" }}>
          <div style={{ background: "#0a0a14", borderRadius: "24px 24px 0 0", padding: "20px 20px 44px", width: "100%", border: "1px solid rgba(124,106,255,0.15)" }}>
            ...
            {[
              ["iPhone/iPad", 'En Safari: tocá <strong style="color:#7c6aff">Compartir ⎋</strong> → <strong style="color:#7c6aff">"Agregar a pantalla de inicio"</strong>'],
              ["Android", 'En Chrome: tocá los <strong style="color:#7c6aff">3 puntitos ⋮</strong> → <strong style="color:#7c6aff">"Agregar a pantalla de inicio"</strong>'],
              ["PC", 'En Chrome: tocá el ícono <strong style="color:#7c6aff">⊕</strong> en la barra de direcciones'],
            ].map(([plat, txt]) => (
              <div key={plat} style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
                ...
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#7c6aff", marginBottom: 3 }}>{plat}</div>
                  <span style={{ color: "#a0a8c0", fontSize: 12, lineHeight: 1.5 }} dangerouslySetInnerHTML={{ __html: txt }} />
                </div>
              </div>
            ))}
