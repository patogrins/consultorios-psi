...
export default function TabPerfil({ usuario, esAdmin, esPublico, t, reservas, onLogin, onLogout }) {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, "usuarios", usuario.email)).then(snap => {
      if (snap.exists()) {
        ...
        setPerfil({ nombre: data.nombre || "", telefono: data.telefono || "", bio: data.bio || "", especialidad: data.especialidad || "", fotoUrl: data.fotoUrl || "" });
      }
    });
  }, [usuario]);
  ...
  async function guardar() {
    ...
    setPerfil(f => ({ ...f, nombre: form.nombre.trim(), telefono: form.telefono.trim(), bio: form.bio.trim(), especialidad: form.especialidad }));
    setGuardando(false);
    setEditando(false);
  ...
  const proximaReserva = useMemo(() => {
    ...
    return misReservas.filter(r => r.fecha >= hoy || r.repeteSemanal).sort((a, b) => a.fecha.localeCompare(b.fecha))[0];
  }, [misReservas]);
  ...
  const inicial = (perfil.nombre || usuario.nombre)?.[0]?.toUpperCase() || "?";

  const inputStyle = { width: "100%", padding: "11px 14px", borderRadius: 12, border: `1px solid ${t.borde}`, fontSize: 13, marginBottom: 12, boxSizing: "border-box", outline: "none", background: t.bgElevated, color: t.texto, fontFamily: "inherit" };
      ...
      <div style={{ padding: "56px 20px 24px", background: "linear-gradient(180deg,#0d0d1a 0%,#000 100%)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div style={{ position: "relative", flexShrink: 0 }}>
            <div style={{ width: 72, height: 72, borderRadius: "50%", background: "linear-gradient(135deg,#667eea,#764ba2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, fontWeight: 800, color: "white", overflow: "hidden", border: "3px solid #1e2235" }}>
              {perfil.fotoUrl
            ...
            {subiendoFoto && (
              <div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ width: 20, height: 20, border: "2px solid #fff2", borderTop: "2px solid white", borderRadius: "50%", animation: "spin 1s linear infinite" }} />
              </div>
            )}
            <button onClick={() => fileRef.current?.click()} style={{ position: "absolute", bottom: 0, right: 0, width: 24, height: 24, borderRadius: "50%", background: t.acento, border: "2px solid #000", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 11 }}>
              ✎
            </button>
        ...
        <div style={{ background: t.bgCard, borderRadius: 16, overflow: "hidden", marginBottom: 14, border: `1px solid ${t.borde}` }}>
          ...
          {!editando ? (
            <div style={{ padding: "12px 16px" }}>
              {[
                { label: "Nombre", value: perfil.nombre || usuario.nombre },
                ...
                <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${t.borde}` }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: value === "No cargado" || value === "No cargada" ? t.textoMuy : t.texto, maxWidth: "60%", textAlign: "right", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{value}</span>
                </div>
              ))}
              {perfil.bio && (
