  ...
  return (
    <div ref={pinchRef} style={{ height:"100vh", overflowY:"auto", background:"#000000" }} className="tab-content">
      ...
      <div style={{ position:"fixed", top:0, left:0, right:0, zIndex:50, background:"rgba(0,0,0,0.92)", backdropFilter:"blur(24px)", WebkitBackdropFilter:"blur(24px)", borderBottom:"1px solid rgba(124,106,255,0.15)", padding:"10px 20px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <button onClick={() => setVerPagos(v => !v)} style={{ padding:"5px 14px", borderRadius:20, border:"1px solid rgba(124,106,255,0.25)", background: verPagos ? "rgba(124,106,255,0.15)" : "transparent", color: verPagos ? "#a78bfa" : "#a0a8c0", fontSize:11, fontWeight:600, cursor:"pointer" }}>
          {verPagos ? "← Agenda" : "💰 Mis reservas"}
        </button>
      </div>
      ...
      {/* ══ FOOTER DINÁMICO (solo en agenda) ══ */}
      {!verPagos && (
        <div style={{ position:"fixed", bottom:90, left:"50%", transform:"translateX(-50%)", zIndex:60, width:"calc(100% - 32px)", maxWidth:360 }}>
          <div style={{ background:"rgba(10,10,20,0.95)", backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", border:"1px solid rgba(124,106,255,0.2)", borderRadius:22, padding:"10px 14px", boxShadow:"0 8px 32px rgba(0,0,0,0.5)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>

              {/* RESUMEN DINÁMICO */}
              <div style={{ flex:1 }}>
                {!pasoActivo ? (
                  <p style={{ margin:0, fontSize:11, color:"#4a5270", textAlign:"center" }}>Tocá un día en la agenda para reservar</p>
                ) : (
                  <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                    {/* Paso 1: día */}
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      <div style={{ width:18, height:18, borderRadius:"50%", background: diaLabel ? "linear-gradient(135deg,#667eea,#764ba2)" : "rgba(124,106,255,0.15)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:800, color:"white", flexShrink:0 }}>1</div>
                      <span style={{ fontSize:12, color: diaLabel ? "white" : "#4a5270", fontWeight: diaLabel ? 700 : 400 }}>
                        {diaLabel || "Elegí un día"}
                      </span>
                    </div>
                    {/* Paso 2: consultorio */}
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      <div style={{ width:18, height:18, borderRadius:"50%", background: consLabel ? "linear-gradient(135deg,#667eea,#764ba2)" : "rgba(124,106,255,0.15)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:800, color:"white", flexShrink:0 }}>2</div>
                      <span style={{ fontSize:12, color: consLabel ? "white" : "#4a5270", fontWeight: consLabel ? 700 : 400 }}>
                        {consLabel ? `${consLabel} — ${flujoConsultorio}` : "Elegí un consultorio"}
                      </span>
                    </div>
                    {/* Paso 3: horas */}
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      <div style={{ width:18, height:18, borderRadius:"50%", background: horasLabel ? "linear-gradient(135deg,#38a169,#2d8a5e)" : "rgba(124,106,255,0.15)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:800, color:"white", flexShrink:0 }}>3</div>
                      <span style={{ fontSize:12, color: horasLabel ? "#66bb6a" : "#4a5270", fontWeight: horasLabel ? 700 : 400 }}>
                        {horasLabel || "Seleccioná los horarios"}
                      </span>
              ...
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4, flexShrink:0 }}>
                <button
                  ...
                  style={{ width:50, height:50, borderRadius:"50%", border:"none", background: flujoHoras.length > 0 ? "linear-gradient(135deg,#667eea,#764ba2)" : "rgba(124,106,255,0.15)", color: flujoHoras.length > 0 ? "white" : "#4a5270", fontSize:28, cursor: flujoHoras.length > 0 ? "pointer" : "not-allowed", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow: flujoHoras.length > 0 ? "0 4px 16px rgba(124,106,255,0.5)" : "none", transition:"all 0.2s" }}>
                  +
                </button>
                {pasoActivo && (
                  <button onClick={resetFlujo} style={{ background:"none", border:"none", color:"#4a5270", fontSize:9, cursor:"pointer", padding:0 }}>cancelar</button>
                )}
              </div>
            </div>
            ...
            {/* Hint de paso */}
            {pasoActivo && (
              <div style={{ marginTop:8, paddingTop:8, borderTop:"1px solid rgba(124,106,255,0.08)", textAlign:"center" }}>
                <span style={{ fontSize:10, color:"rgba(124,106,255,0.6)", fontWeight:600 }}>
                  {paso === "consultorio" ? "→ Ahora tocá un consultorio en la grilla" : paso === "horas" ? "→ Tocá las celdas horarias que querés reservar" : ""}
                </span>
              </div>
            )}
          </div>
