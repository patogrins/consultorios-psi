...
export default function RedGrins({ usuario, t, reservas = [] }) {
  ...
  if (seleccionado) {
    ...
    // Validación segura de si el perfil es del usuario logueado
    const esMio = seleccionado?.email?.toLowerCase().trim() === usuario?.email?.toLowerCase().trim();
    ...
    <div style={{ padding: "16px 14px 100px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {perfilesFiltrados.map(p => {
          ...
          const esMio = p.email?.toLowerCase().trim() === usuario?.email?.toLowerCase().trim();
