  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#000",zIndex:5000,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 14px",display:"flex",flexDirection:"column",gap:10}}>
        {msgs.length===0&&<div style={{textAlign:"center",padding:"40px 0"}}><div style={{fontSize:32,marginBottom:10}}>💬</div><p style={{margin:0,fontSize:13,color:"#4a5270"}}>Empezá la conversación</p></div>}
        {msgs.map(m=>{
...
function FormNuevaFicha({ usuario, onPublicar, onCerrar }) {
  ...
  async function publicar() {
    ...
    await onPublicar({familia:familiaId,subtipo,titulo:form.titulo.trim()||subtipo,especialidad:esp,modalidad:form.modalidad,dias:form.dias,franjas:form.franjas,genero:form.genero,edad:form.edad,nota:form.nota.trim()});
    onCerrar();
  }
  return (
...
function VistaArchivo({ archivadas, setArchivadas, derivaciones, usuario, esAdmin, onVolver, onEliminarDefinitivo }) {
  ...
  function eliminarLocal(id){ const n=archivadas.filter(a=>a!==id); try{const k=`grins_elim_${usuario?.email}`;const e=JSON.parse(localStorage.getItem(k)||"[]");localStorage.setItem(k,JSON.stringify([...e,id]));localStorage.setItem(`grins_arch_${usuario?.email}`,JSON.stringify(n));}catch{} setArchivadas(n); }
  return (
      ...
      {fichas.map(d=>{
        ...
        return (
          <div key={d.id} style={{background:"rgba(14,12,28,0.7)",borderRadius:14,padding:"12px 14px",marginBottom:10,border:"1px solid rgba(255,255,255,0.06)",opacity:0.8}}>
            <div style={{marginBottom:8}}><FamiliaChip subtipo={d.subtipo||"Derivación"} small/><div style={{fontSize:13,fontWeight:700,color:"#4a5270",marginTop:6}}>{d.titulo||d.especialidad||d.subtipo}</div><div style={{fontSize:10,color:"#3a3a5a",marginTop:2}}>por {d.derivadoPor} · {tiempoRelativo(d.creadoEn?.seconds)}</div></div>
...
export default function Derivaciones({ usuario, t, esAdmin, chatInicial, onChatInicialUsado, vistaInicial }) {
  ...
  if(chatAbierto){ const op=perfiles.find(p=>p.email===chatAbierto.otroEmail); return <ChatFullscreen derivacionId={chatAbierto.derivacionId} usuario={usuario} otroNombre={chatAbierto.otroNombre} otroPerfil={op} onCerrar={()=>setChatAbierto(null)}/>; }
