...
function PantallaRegistro({ onIniciar }) {
  ...
  function handleIniciar() {
    ...
    onIniciar(nombre.trim(), email.trim().toLowerCase());
  }
    ...
    <div style={{ minHeight:"100vh", background:"#000", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 20px" }}>
      <div style={{ width:"100%", maxWidth:380 }}>
        ...
        <div style={{ background:"rgba(14,12,28,0.8)", borderRadius:20, padding:"24px", border:"1px solid rgba(124,106,255,0.15)", marginBottom:16 }}>
          ...
          <input type="email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleIniciar()} placeholder="tu@email.com" style={{...inp, marginBottom:0}}/>
        ...
        <div style={{ background:"rgba(124,106,255,0.06)", borderRadius:14, padding:"12px 16px", marginBottom:20, border:"1px solid rgba(124,106,255,0.12)" }}>
          <div style={{ fontSize:12, color:"#a0a8c0", lineHeight:1.6 }}>
            ...
            💾 Tu mejor resultado queda registrado
          </div>
        </div>

        <button onClick={handleIniciar} style={{ width:"100%", padding:"14px", borderRadius:14, border:"none", background:"linear-gradient(135deg,#667eea,#764ba2)", color:"white", fontWeight:800, fontSize:15, cursor:"pointer", boxShadow:"0 4px 20px rgba(124,106,255,0.4)" }}>
          Comenzar evaluación →
        </button>
      </div>
    </div>
...
function Resultado({ score, intento, nombre, email, onReintentar, onVolver, guardando, intentosRestantes }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#000", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 20px", textAlign:"center" }}>
      ...
      {guardando && <p style={{ fontSize:12, color:"#4a5270", marginBottom:16 }}>💾 Guardando resultado...</p>}
...
export default function QuizPsicologia({ onVolver }) {
  const [etapa, setEtapa] = useState("registro"); // registro | quiz | resultado
  const [nombre, setNombre] = useState("");
  const [email, setEmail] = useState("");
  const [intento, setIntento] = useState(1);
  const [mejorScore, setMejorScore] = useState(null);
  ...
  function resetQuiz() {
    setIdx(0); setScore(0); setScoreActual(0);
    setSeleccionado(null); setRespondido(false);
  }
  ...
  async function siguiente() {
    if (idx < PREGUNTAS.length - 1) {
      ...
      // Fin del quiz — guardar resultado
      const notaFinal = scoreActual + (PREGUNTAS[idx].opciones[seleccionado]?.ok ? 0 : 0); // ya sumado en elegir
      const scoreGuardar = scoreActual;
      const esMejor = mejorScore === null || scoreGuardar > mejorScore;
      if (esMejor) setMejorScore(scoreGuardar);
      ...
      try {
        await addDoc(collection(db, "quiz_resultados"), {
          ...
          porcentaje: Math.round((scoreGuardar / PREGUNTAS.length) * 100),
          intento,
          esMejorIntento: esMejor,
  ...
  if (etapa === "resultado") return (
    <Resultado
      score={scoreActual} intento={intento} nombre={nombre} email={email}
