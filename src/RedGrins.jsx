    ...
    return (
      <div style={{ padding: "16px 14px 100px" }}>
        <button
          ...
          style={{ background: "none", border: "none", color: "#7c6aff", cursor: "pointer", fontSize: 13, fontWeight: 600, marginBottom: 14, display: "flex", alignItems: "center", gap: 6, padding: 0 }}>
          ← Volver a la red
        </button>
        ...
        <div style={{ background: "rgba(14,12,28,0.9)", borderRadius: 20, overflow: "hidden", border: "1px solid rgba(124,106,255,0.2)", marginBottom: 14 }}>
          ...
          <div style={{ background: "linear-gradient(180deg,#0a0a18,#0d0d20)", padding: "28px 20px 20px", textAlign: "center" }}>
            <div style={{ width: 80, height: 80, borderRadius: "50%", background: avatarColor(seleccionado.nombre), overflow: "hidden", border: "3px solid rgba(124,106,255,0.4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, fontWeight: 800, color: "white", margin: "0 auto 12px", boxShadow: "0 0 30px rgba(124,106,255,0.2)" }}>
              {seleccionado.fotoUrl
                ? <img src={seleccionado.fotoUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                : inicial}
            </div>
            <h2 style={{ margin: "0 0 4px", fontSize: 20, fontWeight: 800, color: "white" }}>{seleccionado.nombre}</h2>
            {seleccionado.especialidad && (
              <div style={{ fontSize: 13, color: "#7c6aff", fontWeight: 600, marginBottom: 6 }}>{seleccionado.especialidad}</div>
            )}
            <span style={{ background: actividad.bg, color: actividad.color, borderRadius: 20, padding: "3px 12px", fontSize: 11, fontWeight: 700, border: `1px solid ${actividad.color}33` }}>
              {actividad.label}
            </span>
          </div>
          ...
          <div style={{ padding: "16px 20px" }}>
            ...
            {dias.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {dias.map(d => (
                    <span key={d} style={{ background: "rgba(124,106,255,0.12)", color: "#a78bfa", borderRadius: 8, padding: "3px 10px", fontSize: 11, fontWeight: 600, border: "1px solid rgba(124,106,255,0.2)" }}>{d}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "rgba(124,106,255,0.1)", borderRadius: 12, padding: "10px 16px", border: "1px solid rgba(124,106,255,0.2)", textDecoration: "none" }}>
        ...
        {perfilesFiltrados.map(p => {
          ...
          return (
            <div
              ...
              style={{ background: "rgba(14,12,28,0.8)", borderRadius: 16, overflow: "hidden", border: `1px solid ${esMio ? "rgba(124,106,255,0.4)" : "rgba(124,106,255,0.1)"}`, cursor: "pointer", position: "relative", transition: "transform 0.15s" }}
              ...
              <div style={{ padding: "16px 16px 10px", display: "flex", flexDirection: "column", alignItems: "center" }}>
                ...
                {p.especialidad && (
                  <div style={{ fontSize: 10, color: "#7c6aff", fontWeight: 600, textAlign: "center" }}>{p.especialidad}</div>
                )}
              </div>
              ...
              <div style={{ padding: "0 12px 12px" }}>
                ...
                {dias.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 3, justifyContent: "center" }}>
                    {dias.map(d => (
                      <span key={d} style={{ background: "rgba(255,255,255,0.05)", color: "#4a5270", borderRadius: 4, padding: "1px 5px", fontSize: 9, fontWeight: 600 }}>{d}</span>
                    ))}
                  </div>
                )}
              </div>
